...
test_window(mu_Context *ctx)
{
	...
	if (mu_begin_window(ctx, "Demo Window", mu_rect(40, 40, 300, 450))) {
		...
		if (mu_header_ex(ctx, "Background Color", MU_OPT_EXPANDED)) {
			...
			mu_draw_rect(
			    ctx, r, mu_color(bg[0], bg[1], bg[2], 255));
			char buf[32];
			sprintf(buf, "#%02X%02X%02X", (int)bg[0], (int)bg[1],
			    (int)bg[2]);
			mu_draw_control_text(
			    ctx, buf, r, MU_COLOR_TEXT, MU_OPT_ALIGNCENTER);
		}
...
style_window(mu_Context *ctx)
{
	...
	if (mu_begin_window(ctx, "Style Editor", mu_rect(350, 250, 300, 240))) {
		...
		for (int i = 0; colors[i].label; i++) {
			...
			mu_draw_rect(
			    ctx, mu_layout_next(ctx), ctx->style->colors[i]);
		}
		mu_end_window(ctx);
	}
}
...
event(const ev *e)
{
	...
	case CVE_DOWN:
		...
		case CVK_MOUSELEFT:
			mu_input_mousedown(
			    &ctx, cvMouseX(), cvMouseY(), MU_MOUSE_LEFT);
			break;
		case CVK_ESCAPE:
			cvQuit();
			break;
	...
	case CVE_UP:
		...
		break;
	case CVE_UNICODE: {
		uint32_t u = evUnicode(e);
		mu_input_text(&ctx, (const char *)&u);
	} break;
	case CVE_MOTION:
		mu_input_mousemove(&ctx, evX(e), evY(e));
		break;
	case CVE_CLOSE:
		cvQuit();
		break;
